...
async function Site(router, sequelizeObjects) {
  ...
  router.get('/get/latest/object/detection/image', async (req, res) => {
    ...
    const filePath = path.join('/output/object_detection/');
  ...
  router.get('/label/images', async (req, res) => {
    ...
    const filePath = path.join('/output/' + label + '/');
    ...
    try {
      ...
      console.error('Error in /label/images:', e);
      res.status(500);
  ...
  router.delete('/label/image', async (req, res) => {
    ...
    if (id === undefined) {
      ...
      if (row !== null) {
        const filePath = path.join('/output/' + row.label + '/');
  ...
  router.get('/super/resolution/image', async (req, res) => {
    ...
    const filePath = path.join('/output/' + label + '/super_resolution/');
    const stockFilePath = path.join('/output/' + label + '/');
