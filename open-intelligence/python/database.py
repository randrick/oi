...
params = "dbname={} user={} password={} host={} port={}".format(
...
# data table
#                           id:  21007                                (integer, primary key)
#                         name:  HikVision                            (Name of camera)  ? should this be foriegn key?
#                        label:  car                                  (result of object detection)
#                    file_path:  /HikVision/                          (directory path of camera) ? should this be foriegn key?
#                    file_name:  Curbside_00_20230602111208-720.jpg   (image file name)
#             file_create_date:  2024-12-05 18:08:26.676524+00        (file timestamp)
#            file_name_cropped:  2024_12_05_18_08:26_12.jpg
...
#            sr_image_computed:  0
#                sr_image_name:  
# detection_after_sr_completed:  0
#                   email_sent:  0
#        insight_face_computed:  0
...
#            labeling_image_y2:  0
#                      deleted:  f
#                    createdAt:  2024-12-08 22:21:23.730282+00
#                    updatedAt:  2024-12-08 22:21:23.730282+00
...
# TODO: once fully ported, this function should be removed.
def insert_value_old(
    name,
    label,
    file_path,
    file_name,
...
def get_super_resolution_images_to_compute():
    ...
    try:
        ...
        # noinspection SqlDialectInspection,SqlNoDataSourceInspection
        sr_work_query = """
            SELECT id, label, file_name_cropped, detection_result 
            FROM data 
            WHERE 
                file_create_date > now() - interval '1 day' 
                AND (label = 'car' OR label = 'truck' OR label = 'bus') 
                AND sr_image_computed = 0 
            ORDER BY id ASC 
            LIMIT 10"""
...
def get_detection_tasks():
    ...
    try:
        cursor = connection.cursor()
        # noinspection SqlDialectInspection,SqlNoDataSourceInspection
        detection_work_query = """
            SELECT id, label, file_name_cropped 
            FROM data 
            WHERE 
                detection_completed = 0 
                AND detection_result IS NULL 
                AND file_name_cropped IS NOT NULL
...
def get_insight_face_images_to_compute(limit=10):
    connection = psycopg2.connect(params)
    try:
        cursor = connection.cursor()
        # noinspection SqlDialectInspection,SqlNoDataSourceInspection
        ...
        #     WHERE 
        #         label = 'person' 
        #         AND (insight_face_computed = 0 OR insight_face_computed is NULL) 
        #     ORDER BY id ASC """
        query = """
            SELECT id, label, file_name_cropped, detection_result 
            FROM data 
...
def get_images_for_similarity_check_process_after(
    after: str, reverse: bool, limit: int = 0
):
    connection = psycopg2.connect(params)
    try:
        cursor = connection.cursor()
        query = """
            SELECT id, label, file_name_cropped, file_create_date
            FROM data
            WHERE 
              -- label in ('car', 'person') AND
              file_create_date > '"""
        query += after + """' ORDER BY label, file_create_date """
...
def delete_row(id):
    ...
    try:
        cursor = connection.cursor()
        # noinspection SqlDialectInspection,SqlNoDataSourceInspection
        update_query = """DELETE FROM data WHERE id = %s"""
        cursor.execute(update_query, (id,))
        connection.commit()
        cursor.close()
        print('Deleted row: {}'.format(id))
    except psycopg2.DatabaseError as error:
        connection.rollback()
        print(error)
    finally:
...
def delete_rows_with_file_name(file_name):
    connection = psycopg2.connect(params)
    try:
        cursor = connection.cursor()
        # noinspection SqlDialectInspection,SqlNoDataSourceInspection
        update_query = """DELETE FROM data WHERE file_name = %s"""
...
def clean_instances():
    ...
    try:
        ...
        # noinspection SqlDialectInspection,SqlNoDataSourceInspection
        cursor.execute(
            """DELETE FROM instances WHERE "updatedAt" < NOW() - interval '15 minutes'"""
        )
        connection.commit()
        cursor.close()
    except psycopg2.DatabaseError as error:
...
def new_instance(process_name):
    ...
    try:
        ...
        postgres_insert_query = (
            """ INSERT INTO instances (process_name) VALUES (%s) RETURNING id;"""
        )
...
def get_labeled_for_training_lp_images():
    ...
    try:
        ...
        # noinspection SqlDialectInspection,SqlNoDataSourceInspection
        query = """
            SELECT id, label, file_name_cropped, labeling_image_x, labeling_image_y, labeling_image_x2, labeling_image_y2
            FROM data 
            WHERE 
                labeled_for_training = 1 AND 
        ...
        # noinspection SqlDialectInspection,SqlNoDataSourceInspection
        query2 = """
            SELECT id, NULL, file_name_cropped, labeling_image_x, labeling_image_y, labeling_image_x2, labeling_image_y2
            FROM offsites 
            WHERE l
                abeled_for_training = 1 
                AND (label = 'car' OR label = 'truck' or label = 'bus')
                AND labeling_image_x > 0
            ORDER BY id ASC;"""
...
def insert_offsite_value(
    name, label, file_name, year, month, day, hour, minute, second, file_name_cropped
):
    connection = psycopg2.connect(params)
    try:
        cursor = connection.cursor()
        ...
        # Variables
        record_to_insert = (name, label, file_name, file_create_date, file_name_cropped)
...
def get_data_retention_data(days):
    ...
    try:
        ...
        # noinspection SqlDialectInspection,SqlNoDataSourceInspection
        dr_work_query = """
            SELECT id, label, file_name, file_name_cropped, sr_image_name 
            FROM data 
            WHERE 
                file_create_date < now() - interval '(%s) day' 
